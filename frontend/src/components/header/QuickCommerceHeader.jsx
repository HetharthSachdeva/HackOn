                ...
                <div className="flex items-center gap-4">
                    ...
                    <div className="hidden items-center gap-3 rounded-2xl bg-[#151c2b] px-4 py-2 ring-1 ring-white/5 lg:flex">
                        ...
                        <div className="flex items-center gap-1.5">
                            ...
                            <p className="whitespace-nowrap text-xs font-bold text-[#FF9900]">Delivering in {deliveryTime} mins</p>
                        </div>
                    </div>

                    {/* Search */}
                    <form onSubmit={handleSearch} className="relative flex-1">
                        <div className={`relative flex items-center rounded-full bg-[#151c2b] ring-1 transition-all ${isAIMode ? 'ring-2 ring-violet-500/60' : 'ring-white/10 focus-within:ring-[#FF9900]/40'}`}>
                            ...
                            <button
                                ...
                                className={`mr-1.5 flex items-center gap-1 rounded-full px-3 py-1.5 text-xs font-bold transition ${
                                    isAIMode ? 'bg-gradient-to-r from-violet-600 to-fuchsia-500 text-white' : 'bg-white/5 text-gray-400 hover:text-[#FFB145]'
                                }`}
                            >
                    ...
                    <div className="hidden items-center gap-5 md:flex">
                        <Link to="/orders" className="flex items-center gap-1.5 text-sm font-medium text-gray-300 transition hover:text-[#FFB145]">
                        ...
                        <Link to="/cart" className="flex items-center gap-1.5 text-sm font-medium text-gray-300 transition hover:text-[#FFB145]">
                    ...
                    {/* Cart */}
                    <Link to="/cart" className="relative flex items-center gap-1.5 text-sm font-medium text-gray-300 transition hover:text-[#FFB145]">
                        <div className="relative">
                            ...
                            {cartCount > 0 && (
                                <span className="absolute -right-2 -top-2 grid h-5 w-5 place-items-center rounded-full bg-[#FF9900] text-[11px] font-black text-black">
                                    {cartCount}
                                </span>
                            )}
                        </div>
                    ...
                    {userInfo ? (
                        <div className="group relative">
                            <div className="grid h-10 w-10 cursor-pointer place-items-center rounded-full bg-gradient-to-br from-[#FF9900] to-[#E88A00] font-black text-black ring-2 ring-white/10">
                            ...
                            <div className="invisible absolute right-0 top-12 z-50 w-40 rounded-xl bg-[#151c2b] p-2 opacity-0 ring-1 ring-white/10 transition-all group-hover:visible group-hover:opacity-100">
                                ...
                                <button onClick={handleLogout} className="w-full rounded-lg px-3 py-2 text-left text-sm font-medium text-gray-200 hover:bg-white/5 hover:text-[#FFB145]">
                                    Sign out
                                </button>
                            </div>
                        </div>
                        ...
                        <Link to="/signIn">
                            <button className="rounded-full bg-[#FF9900] px-5 py-2 text-sm font-bold text-black transition hover:bg-[#FFB145]">
                                Login
                            </button>
                        </Link>
                    )}
